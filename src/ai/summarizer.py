...
LABELS = {
    ...
    "zh": {
        ...
        "references": "參考連結",
        "tags": "標籤",
        "selected_items": "從 {total} 則內容中篩選出 {selected} 則重要資訊。",
        "empty_analyzed": "已分析 {total} 則內容，但沒有達到重要性門檻的項目。",
        "empty_body": (
            "今日暫無重要動態，可能原因：\n"
            "- 今天關注的資訊來源較平靜\n"
            "- AI 評分門檻設定過高\n"
...
class DailySummarizer:
    ...
    def generate_webhook_overview(
        ...
        if language == "zh":
            header = (
                f"# {labels['header']} - {date}\n\n"
                f"> 從 {total_fetched} 則內容中篩選出 {len(items)} 則重要資訊。\n\n"
                "下面會按新聞逐則發送詳情，你可以只看感興趣的標題。\n\n"
            )
        else:
            header = (
                f"# {labels['header']} - {date}\n\n"
